...
(() => {
  ...
  async function bookmarkWork(workId) {
    const resp = await fetch('/ajax/illusts/bookmarks/add', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-csrf-token': getCsrfToken()
    ...
    if (!resp.ok || json.error) throw new Error('BOOKMARK_FAILED');
  ...
  async function unbookmarkWork(workId, bookmarkId) {
    ...
    const resp = await fetch('/ajax/illusts/bookmarks/delete', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-csrf-token': getCsrfToken()
      },
      body: JSON.stringify({ bookmark_id: String(bookmarkId) }),
      credentials: 'include'
    });
    const json = await resp.json().catch(() => ({}));
    if (!resp.ok || json.error) throw new Error('UNBOOKMARK_FAILED');
    const cached = cache.get(String(workId));
    if (cached) {
