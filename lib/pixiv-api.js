...
(() => {
  ...
  async function fetchWithRateLimit(url, signal) {
    ...
    if (resp.status === 429) {
      ...
      checkAbort(signal);
      lastRequestTime = Date.now();
      const retry = await fetch(url, { credentials: 'include', signal });
  ...
  async function getWorkInfo(workId, { signal } = {}) {
    workId = String(workId);
    checkAbort(signal);
    // Check cache
    const cached = readCache(cache, workId);
    ...
    return enqueueRequest(async () => {
      ...
      const cached2 = readCache(cache, workId);
