...
(() => {
  ...
  async function followUser(userId) {
    const resp = await fetch('/ajax/following/add', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-csrf-token': getCsrfToken()
      },
      body: JSON.stringify({ user_id: String(userId), restrict: 0 }),
      credentials: 'include'
    });
  ...
  async function unfollowUser(userId) {
    const resp = await fetch('/ajax/following/delete', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-csrf-token': getCsrfToken()
  ...
  window.PixivPlusAPI = { getWorkInfo, generateFilename, getUserInfo, followUser, unfollowUser };
})();
